...
const Welcome = () => {
   const {connectWallet,currentAccount,formData,handleChange,sendTransaction,isLoading,transactionCount} = useContext(TransactionContext);
        ...
        <div className='flex flex-col flex-1 items-center justify-start w-full mf:mt-0 mt-10'>
          <div className='p-3 flex justify-end items-start flex-col rounded-xl h-40 sm:w-72 w-full my-5 eth-card .white-glassmorphism'>
            <div className='flex justify-between flex-col w-full h-full'>
              ...
              <div>
                <p className='text-white font-light text-sm'>{currentAccount ? `${currentAccount?.slice(0,5)}....${currentAccount?.slice(currentAccount?.length - 4)}` : "Connect Wallet" }</p>
                <p className="text-white font-light text-sm mt-2">
                  Total Transactions: {currentAccount ? transactionCount : ""}
                </p>
